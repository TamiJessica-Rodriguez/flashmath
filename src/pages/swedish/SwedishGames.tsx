                        ...
                        <div className="space-y-4">
                            {project.tasks.map((task: Task) => (
                                <a
                                    key={task.id}
                                    href={task.link || '#'}
                                    className={`block p-3 rounded-lg ${['Föreläsningar', 'Spel'].includes(project.title) ? 'bg-white shadow-md flex items-center gap-4' : 'bg-gray-100'}`}
                                >
                                    {/* Visa bilder för "Föreläsningar" och "Spel" */}
                                    {['Föreläsningar', 'Spel'].includes(project.title) && task.imageId && (
